...
flags.DEFINE_string('env_name', 'kitchen-mixed-v0', 'environment name')
...
flags.DEFINE_integer('max_path_length', 280, 'Maximum path length')
...
def main(_):
    ...
    import wandb
    import optax
    from dataset.d4rl_kitchen_dataset import D4RLKitchenSeqDataset
    import DV
    ...
    if config.env_name == 'kitchen-mixed-v0':
        config.stride = 4
    elif config.env_name == 'kitchen-partial-v0':
        config.stride = 1
    else:
        raise ValueError('Unknown environment name')

    env = gym.make(config.env_name)
    planner_dataset = D4RLKitchenSeqDataset(
    ...
    policy_dataset = D4RLKitchenSeqDataset(
        env.get_dataset(), horizon=config.planner_horizon, discount=config.discount, 
        stride=config.stride, center_mapping=(config.guidance_type!="cfg")
    )
